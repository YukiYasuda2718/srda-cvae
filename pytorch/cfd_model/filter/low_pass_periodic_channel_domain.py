...
class LowPassFilter:
    ...
    def apply(
        self, hr_gird_data: torch.Tensor, apply_fft_again: bool = False
    ) -> torch.Tensor:
        ...
        if not apply_fft_again:
            return self.fft_lr.apply_ifft2(lr_spec_data)
        else:
            # This is to apply 3/2 rule in psuedo-spectral method.
            _lr = self.fft_lr.apply_ifft2(lr_spec_data)
            _lr_spec = self.fft_lr.apply_fft2(_lr)
            return self.fft_lr.apply_ifft2(_lr_spec)